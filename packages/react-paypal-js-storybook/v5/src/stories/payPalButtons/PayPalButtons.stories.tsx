...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      page: (): JSX.Element => (
        <DocPageStructure
          code={getDefaultCode(meta.args)}
          additionalExamples={[
            { title: "Donate", code: getDonateCode(meta.args) },
          ]}
        />
      ),
    },
  ...
  return (
    <>
      ...
      <PayPalButtons
        ...
        forceReRender={[style]}
        createOrder={() =>
          createOrder([{ sku: "1blwyeo8", quantity: 1 }]).then((orderData) => {
            if (orderData.id) {
...
export const Donate: FC<Omit<StoryProps, "showSpinner" | "fundingSource">> = ({
  ...
  <PayPalButtons
    ...
    createOrder={() =>
      createOrder([{ sku: "etanod01", quantity: 1 }]).then((orderData) => {
        if (orderData.id) {
          action(ORDER_ID)(orderData.id);
          return orderData.id;
        } else {

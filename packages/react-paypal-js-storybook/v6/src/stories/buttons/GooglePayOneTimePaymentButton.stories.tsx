...
1. **Register your checkout domain with Google.** In production, Google validates that the page rendering the button is an allowed domain. Register the **full domain name** where checkout actually runs (e.g. \`shop.example.com\`) in the Google Pay Business Console — it matches the exact host, not just the apex \`example.com\`.

2. **Tell PayPal your checkout origin.** How you do this depends on where eligibility runs:
   - **Server-side** (\`fetchEligibleMethods\`, available as of \`@paypal/react-paypal-js\` 10.3.0): pass \`merchant_info.merchant_origin\` in the payload. There is no browser origin on the server, so you must supply it. Despite the \`origin\` in the name, this must be a **bare hostname** — no scheme, no port (\`shop.example.com\`, **not** \`https://shop.example.com\` or \`shop.example.com:8443\`). A scheme or port makes Google reject it with an \`OR_BIBED\` origin error. See [Eligibility and Hydration](?path=/docs/v6-documentation-eligibility-and-hydration--docs) for details.
   - **Client-side** (\`useEligibleMethods\`): the SDK resolves the origin automatically from the browser (\`window.location.hostname\`, or your client token's domain claim). You don't — and can't — set it. PayPal uses this host when preparing the signed authorization Google requires in production.

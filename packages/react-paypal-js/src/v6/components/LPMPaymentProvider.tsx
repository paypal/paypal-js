...
/**
 * Maps each field type in `Fields` to its `<Capitalize>Field` component key
 * (e.g. `"name"` → `NameField`), typed as a field component rather than
 ...
 * ```
 */
export type LPMEnhancedHookReturn<
  Fields extends readonly LPMFieldType[] = readonly LPMFieldType[],
> = LPMPaymentSessionReturn & {
  /**
...
function createFieldComponent(
  ...
  function FieldComponent({
    ...
    React.useEffect(() => {
      ...
      const s = session;
      if (!s?.createPaymentFields || !container) {
        return;
      }
      container.innerHTML = "";
  ...
  return FieldComponent as React.FC<LPMFieldComponentProps>;
}
...
export function createEnhancedLPMHook<const Fields extends readonly LPMFieldType[]>(
  ...
  return function useLPMEnhancedSession(
    ...
    const fieldComponentsRef =
      useRef<Record<string, React.FC<LPMFieldComponentProps>>>();

    if (!fieldComponentsRef.current) {
      const components: Record<string, React.FC<LPMFieldComponentProps>> = {};
    ...
    } as LPMEnhancedHookReturn<Fields>;
  };
}

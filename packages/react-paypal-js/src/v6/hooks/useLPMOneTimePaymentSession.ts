...
export function useLPMOneTimePaymentSession({
  ...
  useEffect(() => {
    ...
    const newSession = createPaymentSession({
      // orderId is read here (not only later via paymentSessionPromise in
      // handleClick) because the `orderId` prop variant of this hook creates
      // the session immediately with a known order — createSession() needs it
      // up front in that case, so it must stay a dependency of this effect.
      sessionCreator: () =>
        createSession({
          orderId,
          ...proxyCallbacks,

...
describe("useLPMOneTimePaymentSession", () => {
  ...
  describe("sessionFields (T1)", () => {
    ...
    test("should include phone and billingAddress in startOptions for LPMs that require both (zip)", async () => {
      const zipSession = createMockLPMSession();
      const zipSdk = createMockSdkInstance(
        "createZipOneTimePaymentSession",
        zipSession,
      );
      mockPayPalContext({ sdkInstance: zipSdk });

      const phone = { countryCode: "61", nationalNumber: "412345678" };
      ...
      expect(zipSession.start).toHaveBeenCalledWith(
        { presentationMode: "popup" },
        expect.any(Promise),
      );
      const paymentSessionPromise = (zipSession.start as jest.Mock).mock
        .calls[0][1];
      await expect(paymentSessionPromise).resolves.toEqual(
        expect.objectContaining({ phone, billingAddress }),
      );
    });

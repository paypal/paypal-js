...
describe('Reload script', () => {
    ...
    beforeEach(async () => {
        [, sdkResponse, smartButtonsResponse] = await Promise.all([
            page.goto(`${baseURL}/e2e-tests/reload-script/index.html`, {waitUntil: 'networkidle2'}),
            page.waitForResponse(response => response.url().startsWith('https://www.paypal.com/sdk/js')),
            page.waitForResponse(response => response.url().startsWith('https://www.sandbox.paypal.com/smart/buttons'))
        ]);
    });
    ...
    it('should switch currency from USD to EUR', async () => {
        ...
        expect(smartButtonsResponseUpdated.url().includes('currency=EUR')).toBe(true);
    });
});
